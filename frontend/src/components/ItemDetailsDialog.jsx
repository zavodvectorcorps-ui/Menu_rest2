...
import { Plus, Star, Sparkles, Flame, ShoppingBag, Image as ImageIcon, X } from 'lucide-react';
      ...
      <DialogContent className="max-w-md p-0 overflow-hidden max-h-[92vh] flex flex-col" data-testid="item-details-dialog">
        ...
        <div className="p-5 overflow-y-auto space-y-3">
          <div className="flex items-start gap-2 flex-wrap">
            ...
            {item.is_hit && (
              <Badge className="bg-red-500 text-white text-xs"><Star className="w-3 h-3 mr-0.5" />{t('badge_hit')}</Badge>
            )}
            {item.is_new && (
              <Badge className="bg-emerald-500 text-white text-xs"><Sparkles className="w-3 h-3 mr-0.5" />{t('badge_new')}</Badge>
            )}
            {item.is_spicy && (
              <Badge className="bg-orange-500 text-white text-xs"><Flame className="w-3 h-3 mr-0.5" />{t('badge_spicy')}</Badge>
            )}
            {item.is_takeaway && (
              <Badge className="bg-sky-500 text-white text-xs" data-testid="item-details-takeaway"><ShoppingBag className="w-3 h-3 mr-0.5" />{t('badge_takeaway')}</Badge>
            )}
