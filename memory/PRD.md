...
- **Fix — моргание sticky-хедера клиентского меню (P0, DONE)**:
  ...
  - **Fix** (`ClientMenuPage.jsx` строки 23-45): гистерезис. Схлопывать в mini только при `scrollY > 120`, разворачивать обратно только при `scrollY < 40`. Между 40 и 120 состояние не меняется. Реализовано через функциональный `setState((prev) => ...)`, чтобы избежать зависимости от stale-closure.
  - **Verified** через testing-agent на iPhone SE viewport (390x844): в диапазоне 60-100px header стабилен, никакого моргания. Клик по табам категорий, переключение секций Kitchen/Bar, якорные ссылки — без регрессий.
