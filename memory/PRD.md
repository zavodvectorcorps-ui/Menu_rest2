...
- **Контроль цен и маржинальности (P0)** — полностью запущен:
  ...
  - Импорт себестоимости из Caffesta API (`/costs/import-caffesta`).
  - Расчёт маржи по каскаду порогов: позиция → категория → общий (по умолчанию 30%). Статусы `ok/warning/critical`.
  - Telegram-алерты при падении маржи ниже `(порог − 5)%`; включается в «Настройках контроля цен» (свой bot_token + chat_id).
  - Редактирование себестоимости и индивидуального порога для каждой позиции.
- **Caffesta — виды оплаты**: вместо одного `payment_id` теперь редактируемый список `payment_methods: [{name, payment_id, is_default}]`. Звёздочкой помечается «по умолчанию» — он используется при отправке заказов в POS. Старое поле `payment_id` сохранено для обратной совместимости.
- **Caffesta — сравнение по времени**: новая вкладка «Сравнение по времени». 3 готовых пресета (Будни 00:00–02:00, Выходные 03:00–06:00, Выходные 12:00–15:00) + кастомный фильтр (период, тип дня, время С/ДО). Эндпоинт `GET /api/restaurants/{id}/caffesta/time-window` возвращает totals, payments, top_products, by_day.
- **Caffesta — оплата по типам**: секция «Оплата по типам» теперь показывает ВСЕ виды оплат из чеков Caffesta (раньше — только «Наличные/Карта»). Агрегация из `export_sales_shift_day` по полю `payment_type`.
