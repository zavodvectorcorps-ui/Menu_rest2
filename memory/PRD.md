...
## Последнее обновление: 2026-02-13 (часть 2)
...
- **Импорт меню Lunchpad — поддержка 3-х уровней вложенности (P0, DONE)**:
  ...
  - **Root cause**: `parse_lunchpad_data` (`/app/backend/routes/menu.py:633`) обрабатывал только 2 уровня. Внутри подкатегории строка `if sub_item.get("type") != 4: continue` отбрасывала вложенные `type=0` (3-й уровень), а реальные бутылки лежали именно там.
  - **Fix**: парсер переписан рекурсивно. Введены хелперы `_parse_lunchpad_price`, `_parse_lunchpad_dish`, `_parse_lunchpad_banner`, `_walk_lunchpad_items`. Любая глубина дерева сплющивается в плоский список категорий с конкатенированными именами вида `«Родитель — Дитя — Внук»` (наша БД-модель одноуровневая). «Оболочечные» родительские категории без собственных блюд (например, «Барное меню» — все позиции уехали в дочерние) больше не создаются, чтобы не загромождать админку.
  - **Verification на реальном файле пользователя**: 61 категория, 400 dishes + 4 баннера = **404 items (100% от type=4 в файле)**. Пиво: 14, Вина: 22, Виски: 23, Шампанское: 2, Игристые: 7 и т.д. — всё на месте.
  - **Регрессионный тест**: `/app/backend/tests/test_lunchpad_nested_import.py` — 5 кейсов (2-level baseline, 3-level extraction, mixed-level, HTML stripping, реальный файл-семпл).
