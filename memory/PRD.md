...
- **Fix — моргание sticky-хедера клиентского меню (P0, DONE)**:
  ...
  - **Iteration 1 (гистерезис)**: два порога (>120 схлопнуть, <40 развернуть). Устранил моргание, но всё ещё было изменение размера на скролле.
  - **Iteration 2 (финальное решение, по запросу пользователя)**: полностью убран scroll-based schrink. Header теперь **всегда компактный на мобильной версии** (`< md`, 768px) и полноразмерный на десктопе — через Tailwind media queries (`md:` prefix). Никакого state, никакого scroll listener'а, никаких transition height/padding — 0 шансов моргания. Layout полностью стабилен (verified: height=160px до/после скролла, delta=0).
  - **Removed**: `headerMini` state, useEffect со scroll listener'ом, `transition-[padding,width,height]` классы.

### Изменения 2026-02-13 (часть 3) — Якорные ссылки на категории
